...
```
 Diagnose  ──▶  Cluster  ──▶  Intervene   ──▶  Gate + Apply  ──▶   Prove    ──▶  Accept / Rollback
 (RCA the       (group       (one action       (safety gates      (re-eval on     (+ write a
  failures)      failures)    group)            → patch config)    train set)      reflection)
```
