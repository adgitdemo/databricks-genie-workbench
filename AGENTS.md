...
npm run build          # production build into docs/site/build
npm run serve          # serve the production build locally
npm run typecheck      # tsc
```

- Content lives in `docs/site/docs/`, grouped into `getting-started/`, `features/`,
...
**The site content is a port of the flat `docs/*.md` files**, so the two can drift.
When you change a topic doc, update both the canonical `docs/NN-*.md` file and its
counterpart under `docs/site/docs/` (e.g. `docs/04-create-agent.md` ↔
`docs/site/docs/features/create-agent.md`).
